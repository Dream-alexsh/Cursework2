...
def test_get_posts_all():
    posts = get_posts_all()
    assert posts == [{'poster_name': 'leo', 'poster_avatar': 'https://randus.org/avatars/w/c1819dbdffffff18.png',
                     ...
                     {'poster_name': 'leo', 'poster_avatar': 'https://randus.org/avatars/w/c1819dbdffffff18.png',
                      ...
                      'content': 'Пурр-пурр! типичная инстаграмная фотка с котом , книжкой и едой. Но не буду скрывать, что это я: а то вдруг у вас кот тоже такой, тогда вы не увидите этого в своих фото. #кот #котики #инста #инстаграм #любовькживотным #любимыйкот ... Как же я люблю этот момент, когда ты понимаешь, что ты ничего толком не умеешь делать и даже не знаешь, что с этим делать.',
                     ...
                     {'poster_name': 'johnny', 'poster_avatar': 'https://randus.org/avatars/m/00183c7e3c382499.png',
                      ...
                      'views_count': 299, 'likes_count': 134, 'pk': 6},
                     {'poster_name': 'hank', 'poster_avatar': 'https://randus.org/avatars/m/383c7e7e3c3c1818.png',
                     ...
                     {'poster_name': 'larry', 'poster_avatar': 'https://randus.org/avatars/m/81898dbdbdffdb18.png',
                      ...
                      'views_count': 141, 'likes_count': 45, 'pk': 8}], 'Не список'
...
def test_get_posts_by_user():
    ...
    assert user_post == [{'poster_name': 'larry', 'poster_avatar': 'https://randus.org/avatars/m/81898dbdbdffdb18.png',
                          'pic': 'https://images.unsplash.com/photo-1581235854265-41981cb85c88?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=880&q=80',
                          'content': 'Утром проснулся раньше всех – вижу у бассейна на вешалке висит оранжевое пальто. О, думаю – как это мое пальто за мной забралось так далеко – за целых 5000 километров. Присмотрелся – а это зонтик. И как только успел его сюда притащить! За завтраком сижу напротив своего попутчика, и все не решаюсь спросить его: «Может быть, мы все-таки не попутчики? Может, нам надо разъехаться в разные стороны? Вы не боитесь, что я сейчас сбегу?». Он не боится. Он вообще ничего не боится, кроме одного – когда у него в машине не работает сигнализация. А если она не работает, то он садится в машину и продолжает идти своим путем.',
                          'views_count': 366, 'likes_count': 198, 'pk': 4},
                         {'poster_name': 'larry', 'poster_avatar': 'https://randus.org/avatars/m/81898dbdbdffdb18.png',
...
def test_get_posts_by_user_type_error():
    with pytest.raises(TypeError):
        assert get_posts_by_user([])


def test_get_comments_by_post_id():
    comment_user = get_comments_by_post_id(1)
    assert comment_user == [{'post_id': 1, 'commenter_name': 'hanna', 'comment': 'Очень здорово!', 'pk': 1}, {'post_id': 1, 'commenter_name': 'jlia', 'comment': ':)', 'pk': 2}, {'post_id': 1, 'commenter_name': 'ralf', 'comment': 'Класс!', 'pk': 3}, {'post_id': 1, 'commenter_name': 'leo', 'comment': 'Интересно. А где это?', 'pk': 4}], 'не список'


def test_get_comments_by_post_id_type_error():
    with pytest.raises(TypeError):
        assert get_comments_by_post_id()


def test_search_for_posts():
    search_post = search_for_posts('лампочка')
    assert search_post == [{'poster_name': 'johnny', 'poster_avatar': 'https://randus.org/avatars/m/00183c7e3c382499.png',
                      'pic': 'https://images.unsplash.com/photo-1482112252853-a77ee8d9a8cc?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=867&q=80',
                      'content': 'Вот обычная лампочка, которая может стать для тебя новым смыслом жизни.',
                      'views_count': 299, 'likes_count': 134, 'pk': 6}], 'не список'


def test_search_for_posts_type_error():
    with pytest.raises(TypeError):
        assert search_for_posts()


def test_get_post_by_pk():
    post_pk = get_post_by_pk(4)
    assert post_pk == {'poster_name': 'larry', 'poster_avatar': 'https://randus.org/avatars/m/81898dbdbdffdb18.png',
                          'pic': 'https://images.unsplash.com/photo-1581235854265-41981cb85c88?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=880&q=80',
                          'content': 'Утром проснулся раньше всех – вижу у бассейна на вешалке висит оранжевое пальто. О, думаю – как это мое пальто за мной забралось так далеко – за целых 5000 километров. Присмотрелся – а это зонтик. И как только успел его сюда притащить! За завтраком сижу напротив своего попутчика, и все не решаюсь спросить его: «Может быть, мы все-таки не попутчики? Может, нам надо разъехаться в разные стороны? Вы не боитесь, что я сейчас сбегу?». Он не боится. Он вообще ничего не боится, кроме одного – когда у него в машине не работает сигнализация. А если она не работает, то он садится в машину и продолжает идти своим путем.',
                          'views_count': 366, 'likes_count': 198, 'pk': 4}, 'не список'


def test_test_get_post_by_pk():
    with pytest.raises(TypeError):
        assert test_get_post_by_pk({})
